"""
SageMaker entrypoint: first Attack-OOD evaluation for gemma_sppft_normal
(Safety Layers' flagship condition) -- 520 AdvBench goals, each wrapped
in a real-world JailbreakLLMs template (Shen et al. 2024, "Do Anything
Now"). Holds the harmful GOAL constant (AdvBench, same ID anchor as
every other condition) and varies only the wrapper/attack mechanism --
see scripts/build_attack_ood_jailbreakllms.py for how goal and template
are combined and stored separately.

Mirrors entrypoint_harmbench_eval.py exactly (same model source, same
eval pipeline with checkpointing/concurrent-S_h fixes already in
...
CONFIG_FILE = "eval_gemma_sppft_normal_attackood_jbllms.yaml"
...
def main():
    ...
    print("=" * 70, flush=True)
    print(f"[entrypoint] Running {CONFIG_FILE} (Attack-OOD: JailbreakLLMs templates)", flush=True)
    print("=" * 70, flush=True)
    try:
